...
```{r}
library(readxl)
download.file("http://mjwebster.github.io/DataJ/spreadsheets/Police_Incidents_2015.xlsx", "../data/Police_Incidents_2015.xlsx")
incidents <- read_excel("data/Police_Incidents_2015.xlsx", 
    skip = 2)
incidents
```
...
To make that same format we have to introduce a new function called `spread()`.

```{r}
incidents %>%
  group_by(Neighborhood, month(ReportedDate)) %>%
  summarize(crimes=n()) %>%
  spread(key="month(ReportedDate)", value="crimes")
```
...
MLBplayers <- read_excel("../data/MLBpayrolls.xls", sheet = "Twins-Brewers")
MLBplayers
```
...
MLBplayers %>%
  group_by(POSITION) %>%
  summarize(TotalSalary=sum(SALARY), Players=n())
```
...
MLBplayers %>%
  ...
  spread(key=TEAM, value=AverageSalary)
```
...
MLBplayers %>%
  ...
  summarize(AverageSalary=mean(SALARY)) %>%
  spread(key=TEAM, value=AverageSalary) %>%
  transmute(Brewers=dollar(Brewers),Twins=dollar(Twins))
```
...
MLBplayers %>%
  ...
  spread(key=TEAM, value=TotalSalary) %>%
  ungroup %>%
  mutate(Brewers = Brewers/sum(Brewers,na.rm=TRUE), Twins = Twins/sum(Twins,na.rm=TRUE))
...
```{r}
UNCsalaries %>%
  group_by(month=month(hire_date)) %>%
...
UNCsalaries %>%
  group_by(department) %>%
  summarise(AvgSalary=mean(total_salary), employees=n(), maxSalary=max(total_salary), minSalary=min(total_salary))%>%
