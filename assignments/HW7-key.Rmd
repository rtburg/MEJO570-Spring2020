...
```{r}
MLBpayrolls %>%
  mutate(difference=Payroll.2016 - Payroll.2011)
```
...
```{r}
MLBpayrolls %>%
  mutate(pct_change=(Payroll.2016 - Payroll.2011) / Payroll.2011)
```
...
```{r}
#MLBpayrolls %>%
 # mutate(pct_diff=(Payroll.2016 - Payroll.2011) / Payroll.2011) %>%
  #arrange(desc(pct_diff))

#alternate answer:
MLBpayrolls %>%
  mutate(pct_change=(Payroll.2016 - Payroll.2011) / Payroll.2011) %>%
  select(TEAM, pct_change) %>%
  filter(pct_change==max(pct_change))

#Note that the better way to write percent increases of 100% or more is to as a multiple. To find that you would do 
...
MLBpayrolls %>%
  rowwise()%>%
  mutate(avg_team_payroll=mean(Payroll.2004:Payroll.2016))
```
